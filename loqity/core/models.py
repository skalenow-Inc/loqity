...
class User(models.Model):
    ...
    display_picture = models.CharField(max_length=250)
    ...
    def is_valid(self, *args, **kwargs):
        ...
        if "key" in kwargs:
            ...
            tokens = Token.objects.filter(
                key=tokent_str, expires_at__gte=timezone.now()).values("logity_id")
...
class Token(models.Model):
    ...
    def generate_key(self, id_):
        ...
        exists = True
        tokens = Token.objects.filter(
            logity_id=id_, expires_at__gte=timezone.now()).values("key")
        for token in tokens:
        ...
        if existing_token is None:
            ...
            Token.objects.filter(
                logity_id=id_, expires_at__lte=timezone.now()).delete()
        return exists, token
...
class Beacon(models.Model):
    ...
    battery = models.PositiveIntegerField(
        validators=[MaxValueValidator(100), ], null=True, default=None)
    ...
    def save(self, *args, **kwargs):
        ...
        return super(Beacon, self).save(*args, **kwargs)
